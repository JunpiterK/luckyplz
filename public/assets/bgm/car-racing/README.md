...
car-racing 의 오디오 엔진은 **4 트랙 랜덤 셔플** 구조 (dodge 패턴 차용).

- 게임 시작 시 4개 중 랜덤 1곡 선택 → 페이드 인
- 트랙 끝나면 다른 랜덤 트랙으로 즉시 전환 (직전 트랙 제외, 페이드 없음)
- 다음 게임에서도 새로 랜덤 선택 → 연속 플레이도 분위기 다르게
- track2~4 가 없으면 (404) 자동으로 스킵 → 1개만 있어도 동작
- 페이드 인/아웃·일시정지·재개·BGM 토글 로직은 그대로 보존
