...
/* In-app WebView exit helper 로더 (2026-09-25) — 카카오톡·LINE·인스타·페북·
   위챗·텔레그램 등 메신저/SNS 내장 브라우저에서 폰 기본 브라우저로 보낸다.
   본체·앱별 탈출 수단 표는 /js/lpInApp.js. 여기서는 UA 사전검사만 한다 —
   일반 브라우저(Chrome·Safari·삼성인터넷·Firefox·Edge·Whale)는 파일을 받지도
   않는다. 사전검사는 넓게(상위집합) 잡고 정밀 판정은 모듈이 한다.
   openExternalBrowser= 가 붙은 채 일반 브라우저에 도착한 경우(LINE 탈출 직후)도
   로드해서 주소창의 그 파라미터를 지운다. iframe 안에서는 모듈이 스스로 무시. */
try{
    (function(){
        ...
        var hit=mobile&&(
            /KAKAO|\bLine\/|NAVER|Daum|\bBAND\/|everytime|Instagram|Barcelona|FBAN|FBAV|FB_IAB|FBIOS|FB4A|FBSS|Messenger|Orca-Android|MicroMessenger|\sQQ\/|Weibo|musical_ly|Bytedance|trill_|TikTok|Snapchat|Twitter|LinkedInApp|WhatsApp|WA4A|WAiOS|Telegram|Discord|Viber|Zalo|baiduboxapp|; wv\)/i.test(ua)
            ||(/iPhone|iPad|iPod/i.test(ua)&&!/Safari\//.test(ua))
            ||('TelegramWebview' in window)||('TelegramWebviewProxy' in window));
        if(!hit&&!/[?&]openExternalBrowser=1/.test(location.search))return;
        var s=document.createElement('script');
        s.src='/js/lpInApp.js?v=1790311093';
...
(function(){
    ...
    if(!window.LpFullscreen){
        ...
        fs.src='/js/lpFullscreen.js?v=1790311093';
        document.body.appendChild(fs);
    }

    /* 공용 떠 있는 버튼 독 (2026-09-25) — ⛶·🔊·? 를 좌상단 한 줄로 모으고 플레이 중엔 손잡이로 접는다.
       🎮(topNav) 도 플레이·스크롤·주 버튼 겹침 때 숨긴다. 버튼 모듈보다 늦게 와도 붙어 있는 버튼을 옮긴다. */
    if(isGamePage && !window.LpChrome){
        var ch=document.createElement('script');
        ch.src='/js/lpChrome.js?v=1790311093';
        document.body.appendChild(ch);
    }

    /* 도움말 ? 버튼 (2026-09-24) — 게임 아래 긴 글을 화면에서 숨기고 ? 시트 안으로 옮긴다.
       운영자: '스크롤하면 쓸데없는 글이 보인다 — 게임 설정·선택만, 도움말은 버튼 하나로'.
       글은 DOM 에 남아 색인은 유지된다. */
    /* siteFooter 를 defer 없이 긴 글보다 먼저 싣는 게임(버거·행성·델타브이·스페이스-Z·퀴즈)이 있어
       문서를 다 읽은 뒤에 찾는다 */
    (function(){
        function addHelp(){
            ...
            hp.id='lpHelpJs';
            hp.src='/js/lpHelp.js?v=1790311093';
    ...
    if(isGamePage && !window.LpNav){
        ...
        gn.src='/js/lpGameNav.js?v=1790311093';
        document.body.appendChild(gn);
    }
    /* 자리 비움 자동 일시정지 + 계속하기/그만하기 — 조작형 게임이 LpAutoPauseQ 에 등록한다 */
    if(isGamePage && !window.LpAutoPause){
        var ap=document.createElement('script');
        ap.src='/js/lpAutoPause.js?v=1790311093';
        document.body.appendChild(ap);
    }

    /* In-app WebView exit helper → 파일 맨 위 'lpInApp.js 로더' 로 옮김 (2026-09-25).
       푸터 IIFE 를 기다리지 않고 가장 먼저 뜨게 하려는 것. */

    /* Random-shuffle BGM for /games/* pages. Self-skips if no
       /assets/bgm/<gameId>/track*.mp3 files exist, or if the gameId
       is in its internal SKIP_GAMES list (car-racing + dodge + tetris
       ship their own audio engines). 2026-06-11 정책: 페이지 진입
       시 자동재생하지 않는다. 각 게임의 시작 핸들러가 LpBgm.start()
       (모듈 로드 전이면 window.__lpBgmWanted=true) 를 호출할 때만
       재생 시작. */
    if(isGamePage&&!window.LpBgm){
        var bgm=document.createElement('script');
        bgm.src='/js/lpBgm.js?v=1790311093';
        bgm.defer=true;
    ...
    if(isGamePage&&!window.LpWakeLock){
        ...
        wl.src='/js/lpWakeLock.js?v=1790311093';
        document.body.appendChild(wl);
    }
    if(isGamePage&&!window.LpPhaseTimer){
        var pt=document.createElement('script');
        pt.src='/js/lpPhaseTimer.js?v=1790311093';
    ...
    if(!adPolicyOff&&!robotsNoindex&&document.querySelector('[data-lp-ad]')){
        var s=document.createElement('script');
        s.src='/js/adSlots.js?v=1790311093';
        s.defer=true;
    ...
    if(!window.LpRecent){
        ...
        rr.src='/js/recentResults.js?v=1790311093';
    ...
    if(!isGamePage){
        ...
        pwa.src='/js/pwaInstall.js?v=1790311093';
        pwa.defer=true;
    ...
    if(window.supabase){
        ...
        rr2.src='/js/lpRoom.js?v=1790311093';
        rr2.defer=true;
        ...
        hc.src='/js/lpHostCtl.js?v=1790311093';
        hc.defer=true;
        ...
        mp.src='/js/lpMultiplayer.js?v=1790311093';
        mp.defer=true;
    ...
    if(window.supabase&&!window.LpSocial){
        ...
        ls.src='/js/lpSocial.js?v=1790311093';
        ls.defer=true;
    ...
    if(window.supabase&&!window.LpActivity){
        ...
        la.src='/js/lpActivity.js?v=1790311093';
        la.defer=true;
        la.onload=function(){
    ...
    if(window.supabase&&!window.LpPresence){
        ...
        lp.src='/js/lpPresence.js?v=1790311093';
        lp.defer=true;
    ...
    if(window.supabase&&!window.LpInvite){
        ...
        li.src='/js/lpInvite.js?v=1790311093';
        li.defer=true;
    ...
    if(window.supabase&&isGamePage&&!window.LpInviteButton){
        ...
        lib.src='/js/lpInviteButton.js?v=1790311093';
        lib.defer=true;
    ...
    if(window.supabase&&!isGamePage&&!window.LpNotify){
        ...
        ln.src='/js/lpNotify.js?v=1790311093';
        ln.defer=true;
    ...
    if (isGamePage && !window.LpGameText) {
        ...
        gt.src = '/js/lpGameText.js?v=1790311093';
        gt.defer = true;
    ...
    function lpLocalizeHome(){
        ...
        /* 글자 길이가 바뀌면(예: 'Trang chủ') 홈 버튼 높이가 달라질 수 있다 — 좌상단 독(lpChrome)이 그 아래로 다시 선다 */
    ...
    /* storage 이벤트는 타 탭 전용 — 같은 탭의 lang bar 전환은
       lp:langchanged (langBar.js/홈이 발행)로 받는다. */
    document.addEventListener('lp:langchanged', function(){
        try { lpLocalizeHome(); } catch(err){}
    });
