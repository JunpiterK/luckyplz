...
### 4.4 타이포그래피·레이아웃 표준 (모바일 세로 우선, 2026-06-20 제정)

본문은 **모바일 세로 화면에서 한 화면당 정보 밀도**를 최우선으로 맞춘다. 여백·행간이 과하면 한눈에 들어오는 글이 적어 오히려 피로가 가중된다(반대로 너무 빽빽·작아도 안 됨). 근거가 된 가독성 원칙:
- **한 줄 길이(measure)**: 라틴 45~75자(이상 66자), **CJK(한글)는 25~40자**. 너무 짧으면(<25) 시선 이동이 잦아 끊김.
- **행간(line-height)**: WCAG 1.4.8/1.4.12 기준 본문 ≥1.5. CJK는 음절블록이 높아 **1.6 안팎**이 밀도·가독 균형점. 1.8↑은 longform 편집용으로 과함.
- **본문 폰트**: 모바일 16px 하한(그 이하는 자동 확대·가독 저하), 데스크탑 17px.
- **좌우 여백**: 모바일 12~16px. 화면폭을 최대한 본문에 쓰되 가장자리 숨통만 남김.
- **정렬·줄바꿈(중요)**: 본문 문단은 **양쪽정렬**로 오른쪽 끝을 flush 하게 채운다. `.book p`(베이지)·`.post p`(표준) 에 `text-align: justify; word-break: normal; overflow-wrap: break-word; hyphens: auto;`. **`word-break: keep-all` 을 본문 문단에 쓰지 말 것** — 한글을 어절 단위로만 줄바꿈해 오른쪽에 몇 글자씩 빈 공간(rag)이 생기고, 띄어쓰기 없는 중국어판은 오버플로 위험까지 있다. `normal` 로 두면 한글은 음절, CJK는 임의 위치에서 끊어 줄을 끝까지 채우고, `justify` 가 오른쪽을 정확히 맞춘다. 영어판은 `hyphens: auto`(+ `<html lang="en">`) 로 하이픈 처리해야 양쪽정렬 시 단어 사이 강이 안 생긴다. **제목(h1/h2)은 예외** — `keep-all` 유지(제목은 단어 중간 줄바꿈이 어색하므로). 측정 결과 적용 후 줄당 오른쪽 gap 평균 27px→**0px**.

**표준 수치 (베이지 책 테마 = 시리즈 longform 기준)**:

| 항목 | 데스크탑 | 모바일(≤540px) |
|---|---|---|
| body font-size / line-height | 17px / **1.72** | 16px / **1.6** |
| `.book` 좌우 padding | 22px | **14px** |
| 문단 간격 `.book p` margin-bottom | 16px | (상속 16px) |
| 장 간격 `.chapter` margin-top | 40px | (상속 40px) |
| 제목-본문 `.chapter h2` margin-bottom | 18px | (상속 18px) |
| hero padding | 40px 0 36px | (상속) |
