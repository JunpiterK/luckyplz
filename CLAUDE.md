...
- **2026-09-25 B안: 카드에는 아이콘 + 이름만.** "언제 쓰는가" 한 줄(`.game-desc`)은 DOM 에 남기되 시각적으로 숨기고, PC(hover 가능)에서는 마우스오버·포커스 시 게임색 테두리 툴팁(`.lp-tile-tip`)으로 보여 준다. 폰은 설명 없음. 이전 규칙: 카드 내용은 **아이콘 + 이름 + "언제 쓰는가" 한 줄**뿐이다. PLAY 배지는 카드 전체가 버튼이라 중복이고 글자만 늘린다
- **2026-09-25 균일 최대 맞춤(`lpFitTiles`, public/index.html)** — 고정 비율 대신 JS 가 '지금 언어·폭'에서 9장 공통으로 ① 이름이 안 잘리는 최대(하한 12.5px) ② 설명 한 줄 우선·안 되면 두 줄(하한 9.9px) ③ 남는 세로 전부 아이콘(카드 62% 상한, 50% 미만이면 글자 상한을 낮춰 재계산). 하한은 이전 고정값이라 어떤 언어도 예전보다 작아지지 않는다. 폰은 좌우 8px·간격 6px·패딩 5px. 아래 cqw 비율은 스크립트 실패 시 폴백
- **비율은 아이콘 33% : 이름 10.5cqw : 설명 7cqw.** 이름을 13cqw 로 뒀더니 271px 카드에서 27px 가 되어 아이콘 대비 과했다
- **아이콘 = "레트로 장난감 진열장" 9종 한 벌 (2026-09-24).** `scripts/blender/tiles_toybox.py` 한 스튜디오에서 찍은 코팅 플라스틱 장난감 + 잉크 외곽선 → `scripts/build_tiles.py` → `public/assets/tiles/toy-<id>.webp`. **한 종만 이모지·SVG 로 되돌리거나 다른 화풍을 섞지 말 것** — 통일감이 이 개편의 목적이다. 새 타일은 같은 스크립트에 장난감 함수를 추가한다
