#!/usr/bin/env python3
"""Generate polished 1200x630 OG images for the Claude Opus 4.8 blog post.
...
LANGS = {
    "":    {"font": "malgunbd.ttf", "sub": "무엇이 좋아졌나 · 벤치마크 비교"},
    "-en": {"font": "arialbd.ttf",  "sub": "What's New · Benchmarked"},
    "-ja": {"font": "YuGothB.ttc",  "sub": "何が進化したか · ベンチマーク"},
    "-zh": {"font": "msyhbd.ttc",   "sub": "有何提升 · 基准对比"},
}
...
def build(lang_suffix, subtitle):
    ...
    LS = 160
    sw0, sh0 = spark.size
    scale = LS / max(sw0, sh0)
    spark = spark.resize((int(sw0 * scale), int(sh0 * scale)), Image.LANCZOS)
    logo = spark
    LS_W, LS_H = logo.size

    # --- hero block (logo + wordmark) centered as a group near upper-middle ---
    f_claude = font("arialbd.ttf", 46)
    f_opus = font("arialbd.ttf", 132)
    claude_txt = "Claude"
    opus_txt = "Opus 4.8"
    ...
    chips = ["SWE-bench Pro  69.2", "AA Intelligence Index  #1 · 61.4"]
    # measure total
    widths = []
    for c in chips:
    ...
    out = OG / f"claude-opus-4-8{lang_suffix}.png"
...
def main():
    ...
    print("Generating Opus 4.8 OG images:")
