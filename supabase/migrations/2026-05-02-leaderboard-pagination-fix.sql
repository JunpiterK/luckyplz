-- =====================================================================
-- Migration: Leaderboard pagination ambiguity fix
-- Date:      2026-05-02
-- Purpose:   2026-05-02-leaderboard-pagination.sql 의 6 RPC 가
--            "column reference 'rnk' is ambiguous" 에러 발생.
--
-- 원인: RETURNS TABLE 의 output 파라미터 'rnk' 와 CTE 'ranked' 의
--       컬럼 'rnk' 가 동일한 이름 → WHERE rnk >= p_start_rank 시
--       PostgreSQL 이 어느 쪽인지 못 정함.
--
-- 해결: CTE 컬럼을 ranked.rnk 로 명시적 prefix. ORDER BY 도 동일.
--       함수 자체의 signature / return shape 은 그대로 유지.
-- =====================================================================
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_score, ranked.best_foods_eaten, ranked.total_games, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_score, ranked.best_level, ranked.total_games,
                   ranked.total_burgers, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
            limit greatest(1, least(p_limit, 100));
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_score, ranked.total_games, ranked.total_wins, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
            limit greatest(1, least(p_limit, 100));
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_score, ranked.total_games, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
            limit greatest(1, least(p_limit, 100));
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_score, ranked.best_lines, ranked.best_level,
                   ranked.total_games, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
            limit greatest(1, least(p_limit, 100));
...
begin
    ...
    if p_scope = 'world' then
        return query
            ...
            select ranked.rnk, ranked.user_id, ranked.nickname, ranked.avatar_url,
                   ranked.best_duration_ms, ranked.total_completes, ranked.achieved_at
            from ranked
            where ranked.rnk >= p_start_rank
            order by ranked.rnk
            limit greatest(1, least(p_limit, 100));
